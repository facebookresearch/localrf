...
#     return smooth_posesnp.astype(np.float32)
...
#   render_poses = smooth_poses_spline(poses, st=0.3, sr=4)  
...
def generate_spiral_path(poses: np.ndarray,
  ...
  # n_views = 20
  # scale = 30
  # c2ws = poses[5][None].repeat([n_views, 1, 1])
  # baseline = np.linalg.norm(poses[5, :3, 3] - poses[4, :3, 3])
  # for i in range(n_views):
  #     c2ws[i, 1, 3] += baseline * scale * (n_views - i) / n_views
      
      
  n_views = 21
  c2ws = poses[95][None].repeat([n_views, 1, 1])
  R = np.array(
      [   [0.9975510,  0.0000000,  0.0699428,],
          [0.0000000,  1.0000000,  0.0000000,],
          [-0.0699428,  0.0000000,  0.9975510,],], dtype=poses.dtype)
  for i in range(n_views):
      c2ws[i, :3, :3] = np.linalg.matrix_power(R, n_views // 2 - i) @ c2ws[i, :3, :3]
...
def generate_ellipse_path(poses: np.ndarray,
  ...
  render_poses = smooth_poses_spline(poses[..., :3, :], st=0.3, sr=4)
  render_poses = np.concatenate([render_poses, np.zeros_like(render_poses[:, 2:, :])], axis=1)
  render_poses[:, 3, 3] = 1
  ...
  # # R1 = np.array([ [ 0.9858, -0.0347, -0.1646],
  # #                 [ 0.0374,  0.9992,  0.0131],
  # #                 [ 0.1640, -0.0190,  0.9863]], dtype=poses.dtype)
  # # n_views = 21
  # # c2ws = np.repeat(poses[55][None], n_views, axis=0)
  # # print("********* c2ws[i, :3, :3]\n", c2ws[0, :3, :3])
  # # R = np.array(
  # #     [   [0.9975510,  0.0000000,  0.0699428,],
  # #         [0.0000000,  1.0000000,  0.0000000,],
  # #         [-0.0699428,  0.0000000,  0.9975510,],], dtype=poses.dtype)
  # # # R = np.array(
  # # #     [   [0.9987503,  0.0000000,  0.0499792,],
  # # #         [0.0000000,  1.0000000,  0.0000000,],
  # # #         [-0.0499792,  0.0000000,  0.9987503,],], dtype=poses.dtype)
  # # R = (c2ws[0, :3, :3]) @ R @ np.linalg.inv(c2ws[0, :3, :3])
  # # for i in range(n_views):
  # #     c2ws[i, :3, :3] = np.linalg.matrix_power(R, n_views // 2 - i) @ c2ws[i, :3, :3]
  ...
  # scale = 22
  # scale = 100
  # c2ws = np.repeat(poses[25][None], n_views, axis=0) # poses[5][None].repeat([n_views, 1, 1])
  # baseline = np.linalg.norm(poses[25, :3, 3] - poses[24, :3, 3])
  # t = (c2ws[0, :3, :3]) @ np.array([1, 0, 0], dtype=c2ws.dtype)
  # for i in range(n_views):
  #   c2ws[i, :3, 3] += t * baseline * scale * (n_views - i - 20) / n_views

  
  # c2ws = np.concatenate([c2ws, np.zeros_like(c2ws[:, 2:, :])], axis=1)
  # c2ws[:, 3, 3] = 1

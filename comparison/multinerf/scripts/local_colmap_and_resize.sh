...
# Run COLMAP.
rm -rf "$DATASET_PATH"/${DB_PATH}
rm -rf "$DATASET_PATH"/${SPARSE_PATH}


### Feature extraction
colmap feature_extractor \
    --database_path "$DATASET_PATH"/${DB_PATH} \
...
### Feature matching # exhaustive_matcher/sequential_matcher
...
# VOCABTREE_PATH=/data/vocab_tree_flickr100K_words32K.bin
...
mkdir -p "$DATASET_PATH"/${SPARSE_PATH}
colmap mapper \
    --database_path "$DATASET_PATH"/${DB_PATH} \
    --image_path "$DATASET_PATH"/images \
    --output_path "$DATASET_PATH"/${SPARSE_PATH} \
